...
public class DeterministicDiceRoller : MonoBehaviour
{
    ...
    public List<GameObject> dicesToAvoid;
    public List<GameObject> dicesToDestroy; 
    ...
    public void StoreDicesInList()
    {
        ...
        for (int i = 0; i < dicesGameObject.transform.childCount; i++)
        {
            if (dicesGameObject.transform.GetChild(i).gameObject.activeSelf 
                && dicesToAvoid.Contains(dicesGameObject.transform.GetChild(i).gameObject) == false
                && dicesToDestroy.Contains(dicesGameObject.transform.GetChild(i).gameObject) == false)
            {
    ...
    public void StorePosition(GameObject dice)
    {
        ...
        if (returnToOrigin)
        {
            if (dice.gameObject.activeSelf)
            {
                ...
                originalScale.Add(dice.transform.localScale);
            }
        }
    }
    ...
    public void RestorePosition(GameObject dice, int diceindex)
    {
        ...
        if (returnToOrigin)
        {
            ...
            dice.transform.localScale = originalScale[diceindex];
        }
    }
    ...
    public void UseForce(GameObject dice)
    {
        ...
        {
            //Add Force
            //forceDirection = Vector3.back * Random.Range(-400.0f, 400.0f) + Vector3.up * Random.Range(200.0f, 400.0f) + Vector3.right * Random.Range(-400.0f, 400.0f);
            dice.GetComponent<Rigidbody>().AddForce(forceDirection * forceMultiplier);
        }
    }

    public void UseForceRelative(GameObject dice)
    {
        if (addForce)
        {
            //Add Force
            dice.transform.LookAt(forceDirection);
            dice.GetComponent<Rigidbody>().AddForce(dice.transform.forward * forceMultiplier * throwDistance);
        }
    }
    ...
    public void AssignStopDiceAction(Action onDiceStopped)
    {
        this.onDiceStopped = onDiceStopped;
    }
    ...
    public void ClearAndStorePositions()
    {
        originalPosition.Clear();
        originalRotation.Clear();
        originalScale.Clear();
        dicesList.ForEach(StorePosition);
    }

    public void ClearVariablesAndStates()
    {
        dicesList.Clear();
        originalPosition.Clear();
        originalRotation.Clear();
    ...
    public void ClearVariablesAndStatesAndDestroyDice()
    {
        dicesList.ForEach(Destroy);
        dicesToAvoid.ForEach(Destroy);
        dicesList.Clear();
        dicesToAvoid.Clear();
        dicesToDestroy.Clear();
        originalPosition.Clear();
        originalRotation.Clear();
        originalScale.Clear();
    }
    public void AddToAvoidList(List<GameObject> dices)
    {
        foreach (var item in dices)
        {
            dicesList.Remove(item);
            dicesToAvoid.Add(item);
        }
    }
    public void AddToDicesList(List<GameObject> dices)
    {
        dicesToAvoid.RemoveAll(d => d == null);
        
        foreach (var item in dices)
        {
            if(dicesToAvoid.Contains(item))
            {
                dicesToAvoid.Remove(item);
                dicesList.Add(item);
            }
        }

        //TODO: Verify if needed: ClearAndStorePositions();
    }

    public void AddToDestructionList(List<GameObject> dices)
    {
        dicesList.RemoveAll(d => dices.Contains(d));
        dicesToAvoid.RemoveAll(d => dices.Contains(d));
        dicesToDestroy.RemoveAll(d => d == null);
        dicesToDestroy.ForEach(Destroy);
        dicesToDestroy.AddRange(dices);
    }
}
